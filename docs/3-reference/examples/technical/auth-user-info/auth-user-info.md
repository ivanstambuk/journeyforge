...
Use inbound auth (JWT/mTLS/API key) and `httpBindings` to normalise identity into `context`, then use outbound OAuth2 client credentials for downstream calls.
