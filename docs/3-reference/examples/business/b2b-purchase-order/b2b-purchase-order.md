...
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `b2b-purchase-order` journey instance. | `b2bPurchaseOrder_start` | Body: `startRequest` with purchase order id, buyer/supplier, and line items. | `$statusCode == 202` and a `journeyId` is returned. | `journeyId` for the purchase order instance. |
| 2 | `getStatusAfterCreation` | Poll status until the journey is waiting for manager approval. | `b2bPurchaseOrder_getStatus` | Path: `journeyId` from step 1. | `$statusCode == 200`; `currentState == "waitForLevel1Approval"`. | `JourneyStatus` with `phase` and `currentState`. |
