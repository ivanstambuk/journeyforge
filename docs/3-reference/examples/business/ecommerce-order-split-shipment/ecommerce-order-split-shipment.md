...
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `ecommerce-order-split-shipment` journey instance. | `ecommerceOrderSplitShipment_start` | Body: `startRequest` with order id, customer id, items, and shipping address. | `$statusCode == 202` and a `journeyId` is returned. | `journeyId` for the order instance. |
| 2 | `getStatusAfterReservation` | Poll status until inventory reservation has completed. | `ecommerceOrderSplitShipment_getStatus` | Path: `journeyId` from step 1. | `$statusCode == 200`; `currentState` progresses past `reserveInventory`. | `JourneyStatus` with `phase` and `currentState`. |
