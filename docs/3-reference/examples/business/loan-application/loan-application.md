...
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `loan-application` journey instance (synchronous to the first wait step when applicable). | `loanApplication_start` | Body: `startRequest` with applicant and loan details. | `$statusCode == 200`; for auto-approved paths the journey may already be terminal with `JourneyOutcome`, and for underwriter paths `phase == "RUNNING"` with `currentState` progressing past `runChecks`. | `JourneyStatus` or `JourneyOutcome` depending on the path taken. |
| 2 | `getStatusAfterChecks` | (Optional) Re-fetch status until multi-bureau and KYC checks have completed. | `loanApplication_getStatus` | Path: `journeyId` from step 1 (or from `JourneyStatus.journeyId`). | `$statusCode == 200`; `phase` is `Running` and `currentState` progresses past `runChecks`. | `JourneyStatus` with `phase` and `currentState`. |
