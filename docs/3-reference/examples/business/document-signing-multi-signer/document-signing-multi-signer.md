...
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `document-signing-multi-signer` journey in SEQUENTIAL mode. | `documentSigningMultiSigner_start` | Body: `startRequest` with `documentId`, `title`, `signingMode: "SEQUENTIAL"`, ordered signers. | `$statusCode == 202` and a `journeyId` is returned. | `journeyId` for the document instance. |
| 2 | `getStatusBeforeSigning` | Optional status check before collecting signatures. | `documentSigningMultiSigner_getStatus` | Path: `journeyId` from step 1. | `$statusCode == 200`; `phase == "Running"` and `currentState == "submitSignature"`. | `JourneyStatus` with `phase` and `currentState`. |
...
| # | Step ID | Description | Operation ID | Parameters | Success Criteria | Outputs |
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `document-signing-multi-signer` journey in PARALLEL mode. | `documentSigningMultiSigner_start` | Body: `startRequest` with `documentId`, `title`, `signingMode: "PARALLEL"`, signers, and optional `documentExpiryHours`. | `$statusCode == 202` and a `journeyId` is returned. | `journeyId` for the document instance. |
| 2 | `getStatusInProgress` | Poll status while signatures are still being collected. | `documentSigningMultiSigner_getStatus` | Path: `journeyId` from step 1. | `$statusCode == 200`; `phase == "Running"` and `currentState == "submitSignature"`. | `JourneyStatus` with `phase` and `currentState`. |
