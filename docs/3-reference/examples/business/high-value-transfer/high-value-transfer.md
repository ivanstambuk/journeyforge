...
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `high-value-transfer` journey instance (synchronous to the first approval/wait step). | `highValueTransfer_start` | Body: `startRequest` with transfer details. | `$statusCode == 200`; `JourneyStatus.phase` and `currentState` reflect progress through checks and towards `waitForApproval`. | `JourneyStatus` for the transfer instance. |
| 2 | `getStatusAfterChecks` | (Optional) Re-fetch status until fraud and sanctions checks have completed. | `highValueTransfer_getStatus` | Path: `journeyId` from step 1 (or from `JourneyStatus.journeyId`). | `$statusCode == 200`; `phase` is `Running` and `currentState` progresses past `runChecks`. | `JourneyStatus` with `phase` and `currentState`. |
