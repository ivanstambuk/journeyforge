...
| 6 | `getResult` | Retrieve the final case outcome. | `supportCaseSla_getResult` | Path: `journeyId` from step 1. | `$statusCode == 200`, `phase == "SUCCEEDED"` or `phase == "FAILED"`. | `JourneyOutcome` with `status == "RESOLVED"` or `"ESCALATED"` (mirroring `output.status` depending on whether step 4 was called before the SLA fired). |
