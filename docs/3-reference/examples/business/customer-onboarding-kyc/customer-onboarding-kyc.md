...
|---:|---------|-------------|--------------|------------|------------------|---------|
| 1 | `startJourney` | Start a new `customer-onboarding-kyc` journey instance. | `customerOnboardingKyc_start` | Body: `startRequest` with basic identity data. | `$statusCode == 202` and a `journeyId` is returned. | `journeyId` for the customer onboarding instance. |
| 2 | `getStatusAfterProspect` | Poll status until the prospect record is created and the journey is waiting for KYC documents. | `customerOnboardingKyc_getStatus` | Path: `journeyId` from step 1. | `$statusCode == 200`; `currentState == "waitForKycDocuments"`. | `JourneyStatus` with `phase` and `currentState`. |
