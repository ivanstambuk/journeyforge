...
| 3 | `getResult` | (Optional) Re-fetch the configuration outcome once the interactive run has completed. | `dailyOrdersBatchClose_getResult` | Path: `journeyId` from step 1 (or from `JourneyOutcome.journeyId`). | `$statusCode == 200`, `phase == "SUCCEEDED"` or `phase == "FAILED"`. | `JourneyOutcome` with `output.status = "SCHEDULED"` and batch parameters. |
