...
Journeys (`kind: Workflow`) are long‑running executions that are typically “owned” by some
principal (for example, an end user or service). Today, most deployments derive that principal
from the authentication context (for example the `sub` claim in a JSON Web Token) and may copy
it into the workflow `context` for business logic.
...
- “Allow only one running journey per subject for this workflow.”

One option would be to model subject entirely inside the workflow `context` (for example,
...
- Platform features can’t depend on it reliably:
  - There is no stable, spec‑wide location for the subject; different specs could use
    different paths or semantics (`subject`, `userId`, `applicant.id`, etc.).
  - The workflow `context` is mutable by design; workflow logic can overwrite or delete any
...
- Separate from the mutable workflow `context`.
...
- Workflow layer:
  - Workflow specs SHOULD normalise identity into `context` in a consistent, well‑documented
...
- Platform layer (out of scope for the DSL):
  - This ADR explicitly forbids introducing any **first‑class subject field** or standardised,
    cross‑workflow “journey subject” concept at the platform/Journeys API level in the DSL.
  - Deployments MAY implement their own identity‑based policies as internal implementation
    details (for example, using private metadata or store keys), but these MUST NOT surface as
    standard fields in the generic Journeys envelopes or as guarantees that other deployments
    can rely on, and MUST NOT be described as a first-class, cross-workflow “subject” concept.
...
- Keeps identity derivation spec‑driven at the workflow level: workflows define how identity is
  normalised into `context`, making the pattern discoverable without constraining platforms.
- Allows different deployments to choose how they derive and encode identity (JWT `sub`,
...
- Subject‑centric behaviour is less portable between deployments, since the identity key lives
