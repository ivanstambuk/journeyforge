...
- Workflows branch on or transform this result using `choice`, `transform`, and `errorMapping`.

This is appropriate when the workflow needs to *react* to the HTTP outcome (status, body, errors). However, some scenarios only need to:
- Trigger a downstream HTTP side-effect (for example, send a notification, audit, or webhook).
- Not block the workflow on the HTTP response, and
- Not change control flow based on whether the call eventually succeeds or fails.

From the workflow’s perspective, these are “fire-and-forget” calls: send the request and conceptually move on.
...
- `mode: notify` (fire-and-forget from the workflow’s perspective):
  - The engine sends the HTTP request but does not expose any HTTP outcome to the workflow:
...
- When `mode: notify`:
  ...
  - `resiliencePolicyRef` MAY be ignored by runtimes; retries are allowed as an implementation detail but MUST NOT affect control flow.

## Consequences

Positive:
- Workflows can capture common “send-and-continue” patterns (notifications, audit pings, best-effort webhooks) without wasting context fields or pretending to care about the HTTP response.
- The DSL surface remains small: we add a single `mode` flag instead of a new state type or connector.
- Implementations can optimise `notify` calls (for example, buffering or asynchronous IO) while keeping the state machine semantics simple: from the workflow’s point of view, the call never blocks on a response.

Negative / trade-offs:
- The “fire-and-forget” behaviour is best-effort: since the workflow does not see HTTP outcomes, failures can only be observed via logs/metrics. This is by design but must be clearly documented.
- Runtimes must be careful not to leak implementation details (for example, by partially populating a result object) in `notify` mode; the contract is that the workflow never sees call outcomes.
