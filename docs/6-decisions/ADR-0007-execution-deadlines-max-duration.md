...
## Context
Journeys and API endpoints already support per-state timeouts:
...
- SLOs and operational expectations (for example “this flow should complete within 30 seconds”) cannot be captured in the journey definition itself.
- Engine implementations cannot easily clamp per-state timeouts to a remaining budget because the desired global deadline is not part of the DSL.

We want:
- A small, declarative way for authors to specify “how long this journey/API is allowed to run”.
...
Key points:
- `spec.execution` is allowed for both `kind: Journey` and `kind: Api`.
- `maxDurationSec`:
...
- Global deadline:
  - When `maxDurationSec` is configured, the engine starts a logical “deadline timer” when the run begins:
    - Journeys: when `/api/v1/journeys/{journeyName}/start` accepts the request.
...
- Per-state timeouts:
  ...
  - For operations without explicit per-state timeouts, the engine MAY:
...
- Feature 001 of the engine MAY initially treat `spec.execution` as unsupported (for example, by rejecting specs that declare it) if full deadline enforcement is not yet implemented.
- The engine that claims support for the execution-deadline feature SHOULD:
...
- Global execution limits reduce the risk of runaway journeys and APIs hanging indefinitely.
  - Engine implementations get a clear contract for clamping per-state timeouts and surfacing timeouts as a first-class error condition.
...
- Cons:
  ...
  - Implementations must integrate deadline tracking with their schedulers, HTTP clients, and external-input handling, which increases engine complexity.
