...
## Context
The Journeys API exposes workflow instances via JSON envelopes:
...
- Make it **on by default** for all workflows, but allow authors to disable it at the workflow level when needed.
...
- Links are **on by default** (when `spec.httpSurface.links` is absent); workflows can opt-out with `enabled: false`.
...
Runtime guidance:
- For workflows where links are **enabled** (default):
  ...
  - `JourneyStatus` SHOULD include:
    ...
    - One link per active external-input state (for example `/steps/{stepId}`) when such states exist.
     - When the workflow is cancelled via `_links.cancel`, the resulting outcome SHOULD use a dedicated cancellation error code (for example `JOURNEY_CANCELLED`) and follow the general error model (phase + error envelope).
  - `JourneyOutcome` SHOULD include:
    - `_links.self` – the result URL for that journey.
- For workflows where `spec.httpSurface.links.enabled == false`:
...
- Pros:
  ...
  - Provides a per-workflow escape hatch for environments that prefer bare responses.
- Cons:
