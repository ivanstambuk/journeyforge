...
This document defines how each JourneyForge workflow exports an OpenAPI 3.1 YAML contract for external consumers.
It also introduces the export shape for synchronous API endpoints (`kind: Api`).

## Goals
- Provide a stable REST surface for:
  - Journeys (`kind: Workflow`):
...
- Keep the surface uniform across workflows and APIs; per-workflow / per-API variants only specialize names/schemas.

## Paths (canonical)

### Journeys (`kind: Workflow`)
- POST `/api/v1/journeys/{workflowName}/start` → `JourneyStartResponse`
- GET `/api/v1/journeys/{journeyId}` → `JourneyStatus`
- GET `/api/v1/journeys/{journeyId}/result` → `JourneyOutcome` (200 only when terminal)

### Step subresources (when the workflow declares external-input steps)
...
- POST `/api/v1/apis/{apiName}` → synchronous execution
  - Request body: workflow input (no `context` envelope).
  - Response:
...
- `JourneyStartResponse`: `{ journeyId: string, workflowName: string, statusUrl: string, _links?: Links }`
- `JourneyStatus`: `{ journeyId, workflowName, phase: enum[Running,Succeeded,Failed], currentState: string, updatedAt: string(date-time), _links?: Links }`
- `JourneyStepRequest`: `{ eventType: string, payload?: object }`
- `JourneyOutcome`: `{ journeyId, workflowName, phase: enum[Succeeded,Failed], output?: any, error?: { code: string, reason: string }, _links?: Links }`
...
- Presence is controlled per-workflow via `spec.httpSurface.links.enabled`:
  - When omitted or `true`, runtimes SHOULD include `_links` as described above.
  - When explicitly `false`, runtimes SHOULD omit `_links` for that workflow, even though the generic schema still declares `_links` as optional.

### API endpoints (`kind: Api`)

API endpoints reuse the same input/output JSON Schemas as workflows but without the `Journey*` envelope:
- Request body:
...
- For journeys (`kind: Workflow`):
  - The exporter emits one OAS YAML per workflow under `docs/3-reference/examples/oas/<workflow>.openapi.yaml` using the canonical paths with `workflowName` constant baked in.
  - `info.title` is set to `JourneyForge – <workflow>`; tags include `<workflow>`.
...
## Future work
- Add per‑workflow input/output JSON Schemas by sampling or from declared `inputSchemaRef`/`outputSchemaRef` in the spec.
...
## Schema integration
- If `inputSchemaRef` is present, the exporter wraps it as the request body schema at `/journeys/{workflowName}/start` under `context`.
