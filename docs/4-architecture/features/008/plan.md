# Feature 008 – Plan – Scheduled journeys (`task.kind: schedule`)

Status: Draft | Last updated: 2025-11-22

## Increments
- [ ] T-008-01: Finalise ADR-0017 for scheduled journeys and `task.kind: schedule` semantics (based on the earlier journey-as-job scheduling design discussion and this feature spec). (≤90 min)
- [ ] T-008-02: Extend DSL model/parser with `task.kind: schedule` and validation rules (including non-interactive scheduled path check). (≤90 min)
...
- Confusion between interactive and scheduled paths → keep `task.kind: schedule` clearly non-interactive and document patterns in examples.
