# Feature 008 – Scheduled journeys (`task.kind: schedule:v1`) – folded into Feature 001
...
- `docs/6-decisions/ADR-0017-scheduled-journeys-task-schedule.md` – normative design for `task.kind: schedule` / `schedule:v1`.
- `docs/3-reference/dsl.md` – DSL reference for `task.kind: schedule:v1` under the States section.
- `docs/4-architecture/features/001/spec.md` – core engine feature spec, including:
  - Goals that cover parsing/executing `task.kind: schedule:v1`.
