...
|----|-------------|--------------|-----------------|--------------|--------------------|--------|
| FR-004-01 | Opt‑in cookie configuration per spec. | Journey and API specs may declare `spec.cookies.jar` (for downstream jar behaviour) and optionally `spec.cookies.returnToClient` (for emitting `Set-Cookie` towards clients); specs without `spec.cookies` behave exactly as before. | DSL validation ensures that when `spec.cookies` is present, nested fields follow the documented shapes and enums, and that when `spec.cookies.returnToClient` is configured it always declares an explicit `mode`. | Invalid shapes (unknown fields, invalid enums, malformed regex, missing `mode` when `returnToClient` is present) cause spec validation failures with clear error messages. | Count how many specs enable `spec.cookies` and which domains are configured (names only, no values). | DSL ref, Feature 004 |
...
- DSL / Spec:
  ...
  - `spec.cookies.returnToClient.mode: "none" | "allFromAllowedDomains" | "filtered"` (required whenever `spec.cookies.returnToClient` is configured).
