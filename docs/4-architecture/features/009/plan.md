# Feature 009 – Journey DSL Linter – Plan
...
## Increments
- [ ] T-009-01: Design linter architecture (schema + Spectral + Java CLI) and align it with the current DSL reference.
- [ ] T-009-02: Introduce initial JSON Schema for `kind: Journey` / `kind: Api` and validate it against existing examples.
- [ ] T-009-03: Document editor/YAML language server integration for `*.journey.yaml` files.
- [ ] T-009-04: Add an optional Spectral ruleset for additional static checks and wire it into a simple CI step.
- [ ] T-009-05: Define the `journeyforge-lint` CLI surface and Gradle integration without requiring a full implementation yet.
- [ ] T-009-06: Implement the Java CLI linter and semantic rules once the DSL stabilises sufficiently.
