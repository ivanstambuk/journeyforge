...
This feature extends the DSL and engine behaviour for *outbound* HTTP only; inbound auth remains modelled via `spec.policies.httpSecurity`.
...
| FR-003-07 | Token caching semantics. | Engines cache access tokens per policy/effective token request (token endpoint + auth method + form payload) and reuse them across journey instances until expiry (`exp` minus configurable skew). | Behaviour is documented in DSL/feature spec; conformance tests validate reuse vs refresh. | Cache misses or mis‑configurations result only in extra token calls, not incorrect authorisation. | Metrics for cache hit/miss per policy id; token TTL distributions. | Performance requirements |
