...
- [ ] T-001-07: Docs: example journey under `docs/3-reference/` and README pointers.
 - [ ] T-001-08: Finalise ADR-0017 for scheduled journeys and `task.kind: schedule` semantics (based on the earlier journey-as-job scheduling design discussion and the merged Feature 001 spec). (≤90 min)
 - [ ] T-001-09: Extend DSL model/parser with `task.kind: schedule` and validation rules (including non-interactive scheduled path check). (≤90 min)
...
- Confusion between interactive and scheduled paths → keep `task.kind: schedule` clearly non-interactive and document patterns in examples; validate non-interactive scheduled paths statically where possible.
...
- DSL: specs with and without `task.kind: schedule` validate cleanly; invalid schedule blocks produce clear errors.
