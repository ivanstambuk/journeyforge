...
| Q-008 | TBD | How much of OAuth2/token caching semantics should be visible in DSL vs treated as engine configuration? | A) Keep DSL limited to policy shapes + MAY-level caching guidance B) Add explicit TTL/cache hints in `httpClientAuth` C) Treat outbound auth entirely as engine-level concern | Open | 2025-11-21 | See Feature 003 – Outbound HTTP Auth & Secrets |
...
| Q-006 | TBD | Canonical error model for journeys and HTTP tasks (RFC 9457 vs custom) | A) Use RFC 9457 Problem Details as canonical internal error shape and map to JourneyOutcome.error via transformer B) Keep custom `{code,reason}` only C) Allow per-journey pluggable error models | Open | 2025-11-19 | See details below |
| Q-007 | TBD | Global execution deadlines for journeys and APIs (spec.execution.maxDurationSec and onTimeout semantics)? | A) Per-spec `spec.execution` block with `maxDurationSec` and timeout error mapping (preferred) B) Engine/platform-only deadlines with no DSL surface C) Multi-level deadlines (platform, environment, journey) with precedence rules | Open | 2025-11-20 | See details below (ADR-0007) |
...
- Options:
  - Option A (recommended): add an optional `spec.execution` block with `maxDurationSec` and an `onTimeout` mapping; engines clamp per-state timeouts to the remaining budget and surface a well-defined timeout error when the budget is exceeded.
  - Option B: rely solely on platform-level deadlines; keep the DSL agnostic to global execution limits.
- Option C: support multi-level deadlines (platform, environment, journey) with precedence rules; the DSL may express a desired budget but the effective limit is resolved at engine configuration time.
- Decision: <pending>
